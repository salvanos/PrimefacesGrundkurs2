...
public class SigninController implements Serializable {
	...
	public void upload() throws Exception {
    	String timeStamp = new SimpleDateFormat("yyyy_MM_dd_HH_mm_ssss").format(new Date());
    	String fileName = "/tmp/" + timeStamp + file.getFileName();
		Path p = Paths.get(fileName);
		Path f = Files.createFile(p);			
		InputStream input = file.getInputstream();
		Files.copy(input, f, StandardCopyOption.REPLACE_EXISTING);
		setImage(new DefaultStreamedContent(new FileInputStream(fileName)));
    }
}